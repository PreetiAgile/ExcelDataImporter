...
"Worker Email ID",
"Worker Government ID",
"Legacy Manager Worker Update",
...
"Worker Address"
            ...
            var workerOrgMappingEntities = new List<string>
        {
            ...
            start:
                ...
                Console.WriteLine($"Version:48.2,Released On:28/08/2026");
                ...
                Console.WriteLine(string.Join(", ", availableUseCases.Skip(95).Take(5)));
                        ...
                        else
                        {
                            switch (useCaseInput.ToLowerInvariant())
                            {
                                ...
                                case "worker active long term leave transaction":
                                    ...
                                    break;
                                case "worker email id":
                                    var workeremailhandler = new workeremailhandler(config, existingDbCounts,encryptionService);
                                    workeremailhandler.ProcessSheets(useCaseInput, validateOnly, configusername);
                                    break;
                                case "worker government id":
                                    var workergovernmentidhandler = new workergovernmentidhandler(config, existingDbCounts,encryptionService);
                                    workergovernmentidhandler.ProcessSheets(useCaseInput, validateOnly, configusername);
                                    break;
                                case "worker address":
                                    var workeraddresshandler = new Workeraddresshandler(config, existingDbCounts, encryptionService);
                                    workeraddresshandler.ProcessSheets(useCaseInput, validateOnly, configusername);
                                    break;
